...
class operationDatabase():
    def input_new_data_in_using_db():
        with open("data_bank/data_bank.txt", "r") as file:
            id = 1
            for bank in file.readlines():
                ...
                amount = bank[5]
                term = bank[7]
                cur.execute("INSERT INTO data_bank(id, bank_name, deposit_name, percentages, amount, term) VALUES(?, ?, ?, ?, ?, ?)", (id, bank_name, deposit_name, percentages, amount, term))
                id += 1
        con.commit()
